...
namespace Ink_Canvas
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            EnableInMultiTouchModeBtn.Visibility = Visibility.Collapsed;
        ...
        private void timerCheckAutoFold_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            try
            {
                if (WinTabWindowChecker.IsWindowExisted("幻灯片放映", false)) // 处于幻灯片放映状态
                {
                ...
                else if ((WinTabWindowChecker.IsWindowMinimized("希沃白板") && WinTabWindowChecker.IsWindowMinimized("希沃视频展台"))) // 窗口已最小化
                {
                    if (foldFloatingBarByUser == false)
                    {
                        if (isFloatingBarFolded) UnFoldFloatingBar_MouseUp(null, null);
                ...
                else
                {
                    ...
                    {
                        if (isFloatingBarFolded == false) FoldFloatingBar_MouseUp(null, null);
                    }
                }
            }
            catch { }
            /* 自动收纳逻辑如下
            foldFloatingBarByUser = true: 用户进行收纳操作. // 控制保持收纳操作不受希沃软件状态的影响
            unfoldFloatingBarByUser = true: 用户进行展开操作. // 控制允许用户在希沃软件内进行展开操作

            1）进入 PPT 模式：
	            foldFloatingBarByUser = false, unfoldFloatingBarByUser = false;

            2）处于 PPT 放映模式：// 无视希沃软件状态
        ...
        private void InkCanvas_Gesture(object sender, InkCanvasGestureEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (StackPanelPPTControls.Visibility == Visibility.Visible) // PPT手势操作
                    {
                        if (gest.ApplicationGesture == ApplicationGesture.Left)
                        {
                            BtnPPTSlidesDown_Click(BtnPPTSlidesDown, null);
                        }
                        else if (gest.ApplicationGesture == ApplicationGesture.Right)
                        {
        ...
        private void inkCanvas_EditingModeChanged(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (inkCanvas1.EditingMode == InkCanvasEditingMode.Ink || drawingShapeMode != 0) inkCanvas1.ForceCursor = true;
                    else inkCanvas1.ForceCursor = false;
            }
            else inkCanvas1.ForceCursor = false;
        ...
        {
            if (forceEraser == true) BtnColorRed_Click(sender, null);
                else BtnErase_Click(sender, e);
        }
        ...
        {
            BtnUndo.Visibility = status ? Visibility.Visible : Visibility.Collapsed;
            BtnUndo.IsEnabled = status;
        }

        private void TimeMachine_OnRedoStateChanged(bool status)
        {
            BtnRedo.Visibility = status ? Visibility.Visible : Visibility.Collapsed;
        ...
        private void LoadSettings(bool isStartup = true)
        {
            ...
            if (Settings.Startup.IsColorfulViewboxFloatingBar) // 浮动工具栏背景色
            {
                ...
                EnableInMultiTouchModeBtn.Background = gradientBrush;
            ...
            */
            ...
            if (Settings.Canvas != null)
            {
                ...
                ComboBoxPenStyle2.SelectedIndex = Settings.Canvas.InkStyle;
        ...
        private void BtnClear_Click(object sender, RoutedEventArgs e)
        {
            ...
            CancelSingleFingerDragMode("clear");
        }
        ...
        private void CancelSingleFingerDragMode(String mode="pen")
        {
        ...
        private void BtnSwitch_Click(object sender, RoutedEventArgs e)
        {
            if (Main_Grid.Background == Brushes.Transparent)
            {
                if (currentMode == 0)
                {
                    ...
                    //AnimationHelper.HideWithSlideAndFade(GridBackgroundCover);
            ...
            else
            {
                switch ((++currentMode) % 2)
                {
                    ...
                    case 1: //黑板或白板模式
                        currentMode = 1;
                        //AnimationHelper.ShowWithSlideFromBottomAndFade(GridBackgroundCover);
        ...
        private void BtnHideInkCanvas_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (Main_Grid.Background == Brushes.Transparent)
            {
                ...
                //AnimationHelper.ShowWithSlideFromLeftAndFade(Fold_Icon);
                CheckEnableInMultiTouchModeBtnVisibility(false);
                HideSubPanels("cursor");
            }
            else
            {
                AnimationHelper.ShowWithSlideFromLeftAndFade(StackPanelCanvasControls);
                //Fold_Icon.Visibility = Visibility.Collapsed;
                CheckEnableInMultiTouchModeBtnVisibility(true);
            }
        }
        ...
        // 更改颜色选中提示
        private void ChangeColorCheckPrompt(int inkColor)
        {
        ...
        private void ColorSwitchCheck()
        {
            ...
            else
            {
                ...
                drawingShapeMode = 0;
                inkCanvas.EditingMode = InkCanvasEditingMode.Ink;
                CancelSingleFingerDragMode("color");
                forceEraser = false;

                ChangeColorCheckPrompt(inkColor);// 更改颜色选中提示
            }
        ...
        private void BtnColorRed_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                inkCanvas.DefaultDrawingAttributes.Color = Colors.Red;
                BorderPenColorRed.Background = Brushes.Red;
            }
            ColorSwitchCheck();
        }
        ...
        bool isInMultiTouchMode = true; // flase
        private void BorderMultiTouchMode_MouseUp(object sender, MouseButtonEventArgs e)
        {
            if (isInMultiTouchMode)
            {
                ...
                isInMultiTouchMode_Text.Text = (isInMultiTouchMode == true) ? "关" : "开";
                //SymbolIconMultiTouchMode.Symbol = ModernWpf.Controls.Symbol.People;
                //SymbolIconMultiTouchMode2.Symbol = ModernWpf.Controls.Symbol.People;
            }
            else
            {
                inkCanvas.StylusDown += MainWindow_StylusDown;
                ...
                //SymbolIconMultiTouchMode2.Symbol = ModernWpf.Controls.Symbol.Contact;
            }
        }
            ...
            try
            {
                ...
                if (Settings.PowerPointSettings.IsNotifyPreviousPage)
                    Application.Current.Dispatcher.BeginInvoke(() =>
                    {
                        ...
                        try
                        {
                            defaultFolderPath = @"D:\Ink Canvas\Auto Saved - Presentations\Position\";
                        }
                        catch (IOException)
                        {
                            defaultFolderPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) +
                                                    @"\Ink Canvas\Auto Saved - Presentations\Position\";
                        }
        ...
        private void PptApplication_SlideShowBegin(SlideShowWindow Wn)
        {
            ...
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                if (Settings.PowerPointSettings.IsAutoSaveStrokesInPowerPoint)
                {
                    ...
                    try
                    {
                        defaultFolderPath = @"D:\Ink Canvas\Auto Saved - Presentations\Strokes\";
                    }
                    catch (IOException)
                    {
                        defaultFolderPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) +
                                                @"\Ink Canvas\Auto Saved - Presentations\Strokes\";
                    }
        ...
        private void PptApplication_SlideShowEnd(Presentation Pres)
        {
            ...
            {*/
                ...
                File.WriteAllText(folderPath + "/Strokes", previousSlideID.ToString());
        ...
        private void ToggleSwitchShowCanvasAtNewSlideShow_Toggled(object sender, RoutedEventArgs e)
        {
            ...
            Settings.PowerPointSettings.IsShowCanvasAtNewSlideShow = false;// ToggleSwitchShowCanvasAtNewSlideShow.IsOn;
            SaveSettingsToFile();
        }
        ...
        private void ToggleSwitchShowButtonPPTNavigation_OnToggled(object sender, RoutedEventArgs e)
        {
            ...
            Settings.PowerPointSettings.IsShowPPTNavigation = true;// ToggleSwitchShowButtonPPTNavigation.IsOn;
            SaveSettingsToFile();
        ...
        private void ComboBoxPenStyle_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if(sender == ComboBoxPenStyle)
            {
                ...
                ComboBoxPenStyle2.SelectedIndex = ComboBoxPenStyle.SelectedIndex;
            }
            else
            {
                Settings.Canvas.InkStyle = ComboBoxPenStyle2.SelectedIndex;
                ComboBoxPenStyle.SelectedIndex = ComboBoxPenStyle2.SelectedIndex;
            }
            SaveSettingsToFile();
        }
        ...
        */
        ...
        {
            if (!isLoaded) return;
            Settings.PowerPointSettings.IsAutoSaveStrokesInPowerPoint = true;// ToggleSwitchAutoSaveStrokesInPowerPoint.IsOn;
            SaveSettingsToFile();
        }
        ...
        {
            if (!isLoaded) return;
            Settings.PowerPointSettings.IsNotifyPreviousPage = true;// ToggleSwitchNotifyPreviousPage.IsOn;
            SaveSettingsToFile();
        }
        ...
        {
            if (!isLoaded) return;
            Settings.PowerPointSettings.IsNotifyHiddenPage = true;//ToggleSwitchNotifyHiddenPage.IsOn;
            SaveSettingsToFile();
        }
        ...
        {
            if (!isLoaded) return;
            Settings.PowerPointSettings.IsNoClearStrokeOnSelectWhenInPowerPoint = true;// ToggleSwitchNoStrokeClearInPowerPoint.IsOn;
            SaveSettingsToFile();
        }
        ...
        {
            if (!isLoaded) return;
            Settings.PowerPointSettings.IsShowStrokeOnSelectInPowerPoint = true;// ToggleSwitchShowStrokeOnSelectInPowerPoint.IsOn;
            SaveSettingsToFile();
        }
        ...
        {
            if (!isLoaded) return;
            Settings.PowerPointSettings.IsAutoSaveScreenShotInPowerPoint = true;//ToggleSwitchAutoSaveScreenShotInPowerPoint.IsOn;
            SaveSettingsToFile();
        }
        ...
        private void ToggleSwitchEnableFingerGestureSlideShowControl_Toggled(object sender, RoutedEventArgs e)
        {
            ...
            Settings.PowerPointSettings.IsEnableFingerGestureSlideShowControl = true;// ToggleSwitchEnableFingerGestureSlideShowControl.IsOn;
        ...
        private void BtnUndo_Click(object sender, RoutedEventArgs e)
        {
            ...
            //MessageBox.Show("" + item.CommitType);
        ...
        private void BtnRedo_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                // 恢复被删除的图片元素
                Image imageToRestore = CreateImageElementFromBytes(item.ImageBytes, item.Width, item.Height, item.ImageId);
                // 设置图片的位置
                System.Windows.Controls.Canvas.SetLeft(imageToRestore, item.X);
                System.Windows.Controls.Canvas.SetTop(imageToRestore, item.Y);
                // 添加图片到画板
                inkCanvas.Children.Add(imageToRestore);
            }
            _currentCommitType = CommitReason.UserInput;
        }

        private Image CreateImageElementFromBytes(byte[] imageBytes, double width, double height, object imageTag)
        {
            ...
            bitmapImage.DecodePixelWidth = (int)width; // 设置图片的宽度
            bitmapImage.DecodePixelHeight = (int)height; // 设置图片的高度
            bitmapImage.EndInit();
        ...
        private void SymbolIconPinBorderDrawShape_MouseUp(object sender, MouseButtonEventArgs e)
        {
            ...
            BoardToggleSwitchDrawShapeBorderAutoHide.IsOn = !ToggleSwitchDrawShapeBorderAutoHide.IsOn;
        ...
        private void BtnScreenshot_Click(object sender, RoutedEventArgs e, bool isHideNotification = false)
        {
        ...
        public void ShowNotification(string notice, bool isShowImmediately = true)
        {
            ...
            new Thread(new ThreadStart(() =>
            {
                Thread.Sleep(notificationShowTime + 500);
        ...
        private void EnableInMultiTouchMode_MouseUp(object sender, MouseButtonEventArgs e)
        {
        ...
        private void CheckChangeTheme()
        {
            ...
            else if (inkCanvas.DefaultDrawingAttributes.Color == StringToColor("#FFFEFEFE"))
            {
                inkCanvas.DefaultDrawingAttributes.Color = Colors.Black;
            }
             */
            if (ViewboxBtnColorBlackContent.Visibility == Visibility.Visible)
            {
                BtnColorWhite_Click(null, null);
            }
            else if (BoardViewboxBtnColorWhiteContent.Visibility == Visibility.Visible)
            {
                BtnColorBlack_Click(null, null);
            }
        }

        private void ToggleSwitchUsingWhiteboard_Btn_Click(object sender, RoutedEventArgs e)
        {
            //ToggleSwitchUsingWhiteboard.IsOn = !ToggleSwitchUsingWhiteboard.IsOn;
            ToggleSwitchUsingWhiteboard_Toggled(null,null);
            CheckChangeTheme();
        }

        private void CheckEnableInMultiTouchModeBtnVisibility(bool isVisible)
        {
            if (isVisible == true)
            {
                if (BtnPPTSlideShowEnd.Visibility == Visibility.Visible) EnableInMultiTouchModeBtn.Visibility = Visibility.Collapsed;
                    else AnimationHelper.ShowWithSlideFromBottomAndFade(EnableInMultiTouchModeBtn);
            }
            else EnableInMultiTouchModeBtn.Visibility = Visibility.Collapsed;
        }
        ...
        private async void FoldFloatingBar_MouseUp(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                if (foldFloatingBarByUser && inkCanvas.Strokes.Count > 1)
                { // && MessageBox.Show("是否继续将 Ink Canvas 收纳至侧边栏？这将清除当前批注的所有墨迹（可撤销恢复）", "Ink Canvas", MessageBoxButton.OKCancel, MessageBoxImage.Warning) != MessageBoxResult.OK)
                    ShowNotification("正在清空墨迹并收纳至侧边栏，如果误触可进入批注模式后通过【撤销】功能来恢复原先墨迹。");
                }
            }
            if (isFloatingBarChangingHideMode) return;
            if (sender == hiddenButtonInBorderTools) BorderTools.Visibility = Visibility.Collapsed;
            await Dispatcher.InvokeAsync(() =>
            {
        ...
        private async void HideSubPanels(String mode=null)
        {
            ...
            if (mode != null)
            {
                ...
                /*if (mode == "cursor")
                    Cursor_Icon.Background = new ImageBrush(new BitmapImage(new Uri("pack://application:,,,/Resources/Icons-png/check-box-background.png"))){Opacity = 0.7 };*/
        ...
        private async void CursorIcon_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (BtnPPTSlideShowEnd.Visibility != Visibility.Visible) //非 PPT 模式下
                {
                    if (isLoaded && Settings.Automation.IsAutoClearWhenExitingWritingMode)
                    {
                        if (inkCanvas.Strokes.Count > 0)
        ...
        private void PenIcon_Click(object sender, RoutedEventArgs e)
        {
            if (!isInMultiTouchMode)
            {
                EnableInMultiTouchMode_MouseUp(null, null);
            }
        ...
        private async void SymbolIconScreenshot_MouseUp(object sender, MouseButtonEventArgs e)
        {
            ...
            await Task.Delay(50);
            BtnScreenshot_Click(null, null, true);*/
        }
        ...
        private async void ImageBlackboard_MouseUp(object sender, MouseButtonEventArgs e)
        {
            ...
            if (currentMode == 0) //进入黑板
            {
                if (Not_Enter_Blackboard_fir_Mouse_Click)
                {// BUG-Fixed：程序启动后直接进入白板会导致后续撤销功能、退出白板无法恢复墨迹
                ...
                await Task.Delay(10);
                Topmost = false;
                if (Settings.Canvas.UsingWhiteboard) BtnColorBlack_Click(BorderPenColorBlack, null);
                else BtnColorWhite_Click(BorderPenColorWhite, null);
                new Thread(new ThreadStart(() =>
                {
                    Thread.Sleep(50);
                    Application.Current.Dispatcher.Invoke(() =>
        ...
        private async void SymbolIconTools_MouseUp(object sender, MouseButtonEventArgs e)
        {
        ...
        void SymbolIconEmoji_MouseMove(object sender, MouseEventArgs e) //移动菜单栏
        {
        ...
        private void SaveInkCanvasStrokesByUser(bool newNotice = true)
        {
            try
            {
                string savePath;
        ...
        private void SymbolIconOpenStrokes_MouseUp(object sender, MouseButtonEventArgs e)
        {
            ...
            try
            {
                defaultFolderPath = @"D:\Ink Canvas\User Saved - BlackBoard Strokes";
            }
            catch (IOException)
            {
                defaultFolderPath = Environment.GetFolderPath(Environment.SpecialFolder.MyPictures) + @"\Ink Canvas\Auto Saved - BlackBoard Strokes";
            }
    ...
    public class WinTabWindowChecker
    {
    ...
    public static class AnimationHelper
    {
        public static void ShowWithSlideFromBottomAndFade(UIElement element, double duration = 0.15)
        {
            ...
            {
                From = element.RenderTransform.Value.OffsetY + 10, // 滑动的距离
                To = 0,
                Duration = TimeSpan.FromSeconds(duration)
            };
        ...
        public static void ShowWithSlideFromLeftAndFade(UIElement element, double duration = 0.5)
        {
            ...
            var slideAnimation = new DoubleAnimation
            {
                From = element.RenderTransform.Value.OffsetX - 20, // 从左到右的滑动距离
                To = 0,
                Duration = TimeSpan.FromSeconds(duration)
            };
        ...
        public static void HideWithSlideAndFade(UIElement element, double duration = 0.15)
        {
            ...
            {
                From = 0,
                To = element.RenderTransform.Value.OffsetY + 10, // 滑动的距离
                Duration = TimeSpan.FromSeconds(duration)
            };
            Storyboard.SetTargetProperty(slideAnimation, new PropertyPath("(UIElement.RenderTransform).(TranslateTransform.Y)"));
